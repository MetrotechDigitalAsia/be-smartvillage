...
<div class="mail-body" style="padding: 0 42px;" >
    ...
    <p style="text-align: justify; font-size: 12px;margin: 0;" >Bahwa Atas dasar suka sama suka mereka telah melangsungkan tata cara perkawinan secara <br>agama Hindu di {{ $field->marriage_location }}, pada hari {{ !empty($field->marriage_date) ? Carbon\Carbon::parse($field->marriage_date)->translatedFormat('l') : '' }} tanggal : {{ Carbon\Carbon::parse($field->marriage_date)->translatedFormat('d/m/Y') }} , dan berkedudukan sebagai PURUSA, adalah : {{ $field->subject_1->name ?? '' }} .</p>
