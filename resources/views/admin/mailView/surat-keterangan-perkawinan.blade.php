...
<div class="mail-body" style="padding: 0 42px;" >
    ...
    <p style="text-align: justify; font-size: 12px;margin: 0;" >Bahwa Atas dasar suka sama suka mereka telah melangsungkan <b>tata cara perkawinan</b> secara <br>agama Hindu di <span style="text-transform: capitalize;" >{{ $field->marriage_location }}</span>, pada hari {{ !empty($field->marriage_date) ? Carbon\Carbon::parse($field->marriage_date)->translatedFormat('l') : '' }} tanggal : {{ Carbon\Carbon::parse($field->marriage_date)->translatedFormat('d/m/Y') }} , dan berkedudukan sebagai PURUSA, adalah : {{ $field->subject_1->name ?? '' }} .</p>
