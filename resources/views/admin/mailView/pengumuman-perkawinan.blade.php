<div style="padding: 0 30px;" >
    ...
    <p style="text-align: justify;" >Agama {{ $field->subject_2->religion ?? '' }}, lahir di Br.{{ $field->subject_2->banjar  ?? ''}}, <span style="text-transform: capitalize;" >{{ $field->subject_2->birthplace ?? '' }}</span>, Tgl. {{ !empty($field->subject_2->birthdate) ? Carbon\Carbon::parse($field->subject_2->birthdate)->translatedFormat('d/m/Y') : '' }}, pekerjaan <span style="text-transform: capitalize;" >{{ $field->subject_2->job ?? '' }}</span>, bertempat tinggal di Br. {{ $field->subject_2->banjar  ?? ''}}, Desa Getasan, Kecamatan Petang, Kabupaten Badung, Provinsi Bali, dahulu sudah/belum pernah kawin dengan - , anak laki-laki dari : <span style="text-transform: capitalize;" >{{ $field->subject_2_father_name }}</span> umur {{ $field->subject_2_father_age ?? '' }} tahun, pekerjaan <span style="text-transform: capitalize;" >{{ $field->subject_2_father_job ?? '' }}</span> dan <span style="text-transform: capitalize;" >{{ $field->subject_2_mother_name }}</span> umur {{ $field->subject_2_mother_age ?? '' }} tahun, pekerjaan <span style="text-transform: capitalize;" >{{ $field->subject_2_mother_job ?? '' }}</span>, keduanya bertempat tinggal di <span style="text-transform: capitalize;" >{{ $field->subject_2_father_address ?? '' }}</span>.</p>
