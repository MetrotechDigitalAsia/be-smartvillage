...
<body>
    ...
    <div class="mail-body" style="width: 650px; margin: inherit auto;" >
        ...
        <p style="text-align: justify; font-size: 12px; text-indent: 0; margin-bottom: 40px;" >Nama Istri : <span style="text-transform: capitalize;" >{{ $field->subject_2->name ?? '' }}</span>, Agama Hindu, Tempat/ tanggal lahir : <span style="text-transform: capitalize;" >{{ $field->subject_2->birthplace ?? '' }}</span>, {{ !empty($field->subject_2->birthdate) ? Carbon\Carbon::parse($field->subject_2->birthdate)->format('d/m/Y') : '' }}, Pekerjaan <span style="text-transform: capitalize;" >{{ $field->subject_2->job ?? '' }}</span>, alamat tempat tinggal di : Banjar/Lingk : <span style="text-transform: capitalize;" >{{ $field->subject_2_banjar ?? '' }}</span>, Desa/Kelurahan {{ $field->subject_2_village ?? '' }}, Kecamatan {{ $field->subject_2_district ?? '' }}, Kabupaten/Kota <span style="text-transform: capitalize;" >{{ $field->subject_2_city }}</span>, Provinsi <span style="text-transform: capitalize;" >{{ $field->subject_2_province }}</span>. Anak Perempuan dari: nama (ayah) <span style="text-transform: capitalize;" >{{ $field->subject_2_father_name }}</span> dan nama (ibu) <span style="text-transform: capitalize;" >{{ $field->subject_2_mother_name }}</span>  bertempat tinggal di : Banjar/Lingk : <span style="text-transform: capitalize;" >{{ $field->subject_2_banjar ?? '' }}</span>, Desa/Kelurahan {{ $field->subject_2_village ?? '' }}, Kecamatan {{ $field->subject_2_district ?? '' }}, Kabupaten/Kota <span style="text-transform: capitalize;" >{{ $field->subject_2_city }}</span>, Provinsi <span style="text-transform: capitalize;" >{{ $field->subject_2_province }}</span>.</p>
        <p style="text-align: justify; font-size: 12px; margin: 0; text-indent: 35px;" >Bahwa Atas dasar suka sama suka mereka telah melangsungkan <b>tata cara perkawinan</b> secara agama Hindu di <span style="text-transform: capitalize;" >{{ $field->marriage_location ?? '' }}</span> pada hari {{ !empty($field->marriage_date) ? Carbon\Carbon::parse($field->marriage_date)->translatedFormat('l') : '' }} tanggal : {{ Carbon\Carbon::parse($field->marriage_date)->translatedFormat('d/m/Y') }} , dan berkedudukan sebagai PURUSA, adalah : {{ $field->subject_1->name ?? '' }} .</p>
    ...
    <div class="mail-footer" style="padding-bottom: 25px; width: 650px; margin: inherit auto;" >
        <table style="font-size: 12px;width: 100%; margin: 0 auto; margin-top: 40px; text-align: center;" >
            <tr>
                ...
                <td style="padding-bottom: 50px; text-align: center;" >Bendesa Adat Getasan,</td>
            </tr>
            <tr>
                <td>(................................................)</td>
