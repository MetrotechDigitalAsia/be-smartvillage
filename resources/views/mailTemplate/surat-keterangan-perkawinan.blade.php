...
<body>
    ...
    <div class="mail-body" style="width: 650px; margin: inherit auto;" >
        ...
        <p style="text-align: justify; font-size: 12px; margin: 0; text-indent: 35px;" >Bahwa Atas dasar suka sama suka mereka telah melangsungkan <b>tata cara perkawinan</b> secara agama Hindu di {{ $field->marriage_location ?? '' }} pada hari {{ !empty($field->marriage_date) ? Carbon\Carbon::parse($field->marriage_date)->translatedFormat('l') : '' }} tanggal : {{ Carbon\Carbon::parse($field->marriage_date)->translatedFormat('d/m/Y') }} , dan berkedudukan sebagai PURUSA, adalah : {{ $field->subject_1->name ?? '' }} .</p>
