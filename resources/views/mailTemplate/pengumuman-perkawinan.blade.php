...
<body style="padding: 0 30px;" >
    ...
    <p style="margin-top: 0; text-align: justify; line-height: 22px;" >Agama {{ $field->subject_2->religion ?? '' }}, lahir di Br.{{ $field->subject_2->banjar  ?? ''}}, {{ $field->subject_2->birthplace ??'' }}, Tgl. {{ !empty($field->subject_2->birthdate) ? Carbon\Carbon::parse($field->subject_2->birthdate)->translatedFormat('d/m/Y') : ''}}, pekerjaan {{ $field->subject_2->job ?? '' }}, bertempat tinggal di Br. {{ $field->subject_2->banjar  ?? ''}}, Desa Getasan, Kecamatan Petang, Kabupaten Badung, Provinsi Bali, dahulu sudah/belum pernah kawin dengan - , anak laki-laki dari : <span style="text-transform: capitalize;" >{{ $field->subject_2_father_name }}</span>  umur {{ $field->subject_2_father_age ?? '' }} tahun, pekerjaan <span style="text-transform: capitalize;" >{{ $field->subject_2_father_job ?? '' }}</span> dan <span style="text-transform: capitalize;" >{{ $field->subject_2_mother_name }}</span> umur {{ $field->subject_2_mother_age ?? '' }} tahun, pekerjaan <span style="text-transform: capitalize;" >{{ $field->subject_2_mother_job ?? '' }}</span>, keduanya bertempat tinggal di <span style="text-transform: capitalize;" >{{ $field->subject_1_father_address ?? '' }}</span>.</p>
